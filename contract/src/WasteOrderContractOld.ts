...
export class WasteOrderContract extends Contract {
    ...
    @Transaction()
    public async commissionWasteOrder(ctx: Context, orderId: string, wasteOrderValue: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
        ...
        let validationResult = Joi.validate(wasteOrder, WasteOrderPublicCommissionSchema);
    ...
    @Transaction()
    public async acceptWasteOrder(ctx: Context, orderId: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
    ...
    @Transaction()
    public async rejectWasteOrder(ctx: Context, orderId: string, updatedWasteOrderValue: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
    ...
    @Transaction()
    public async cancelWasteOrder(ctx: Context, orderId: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
        let wasteOrder = await this.getWasteOrder(ctx, orderId);
        let wasteOrderPrivate = await this.getWasteOrderPrivate(ctx, orderId);
        const MSPID = ctx.clientIdentity.getMSPID();
    ...
    @Transaction()
    public async completeWasteOrder(ctx: Context, orderId: string, updatedWasteOrderValue: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
        let updatedWasteOrderPrivate: WasteOrderPublic = JSON.parse(updatedWasteOrderValue);
    ...
    @Transaction()
    public async correctWasteOrder(ctx: Context, orderId: string, updatedWasteOrderValue: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
        let updatedWasteOrderPrivate: WasteOrderPublic = JSON.parse(updatedWasteOrderValue);
    ...
    @Transaction(false)
    public async getCompleteWasteOrder(ctx: Context, orderId: string): Promise<WasteOrderPublic & WasteOrderPrivate> {
    ...
    private async getWasteOrder(ctx: Context, orderId: string): Promise<WasteOrderPublic> {
        const exists = await this.checkIfWasteOrderExists(ctx, orderId);
        if (!exists) {
            throw new Error(`The order ${orderId} does not exist`);
        }
    ...
    @Transaction(false)
    public async getWasteOrdersForSubcontractorWithStatus(ctx: Context, MSPID: string, status: string): Promise<(WasteOrderPublic & WasteOrderPrivate)[]> {
        let wasteOrderResults: (WasteOrderPublic & WasteOrderPrivate)[] = [];
    ...
    @Transaction(false)
    public async getWasteOrdersForOriginatorWithStatus(ctx: Context, MSPID: string, status: string): Promise<(WasteOrderPublic & WasteOrderPrivate)[]> {
        let wasteOrderResults: (WasteOrderPublic & WasteOrderPrivate)[] = [];
    ...
    private async saveWasteOrder(ctx: Context, wasteOrder: WasteOrderPublic, wasteOrderPrivate: WasteOrderPrivate) {
        ...
        await this.createContext().stub.putPrivateData("OrderingOrgMSP-SubcontractorOrgMSP", wasteOrderPrivate.id, wasteOrderPrivateBuffer);
    }

    private async getWasteOrdersFromIterator(iterator: Iterators.StateQueryIterator): Promise<WasteOrderPublic[]> {
        let wasteOrders: WasteOrderPublic[] = [];

        while (true) {
            let result = await iterator.next();

            if (result.value && result.value.value.toString()) {
                let wasteOrder: WasteOrderPublic;
