...
docker exec cli peer chaincode instantiate -n Wastechain -v 0 -l node -c '{"Args":["init"]}' -C wastechain -P "OR('OrderingOrgMSP.peer', 'SubcontractorOrgMSP.peer', 'ThirdPartyOrgMSP.peer')" --collections-config "/opt/gopath/src/github.com/hyperledger/fabric/peer/config/private-data-collections.json"