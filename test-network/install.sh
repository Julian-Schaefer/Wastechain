...
docker exec -e "CORE_PEER_LOCALMSPID=OrderingOrgMSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/crypto-config/peerOrganizations/ordering-organisation.com/users/Admin@ordering-organisation.com/msp" -e "CORE_PEER_ADDRESS=peer0.ordering-organisation.com:7051" cli peer chaincode instantiate -n Wastechain -v 1.0 -l node -c '{"Args":["init"]}' -C wastechain -P "OR(AND('OrderingOrgMSP.peer', 'SubcontractorOrgMSP.peer'), AND('ThirdPartyOrgMSP.peer', 'SubcontractorOrgMSP.peer'), AND('OrderingOrgMSP.peer', 'ThirdPartyOrgMSP.peer'))" --collections-config "/etc/hyperledger/config/private-data-collections.json"
